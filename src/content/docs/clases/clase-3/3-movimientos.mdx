...
Como vemos en este ejemplo, la variable `x` que es la que le da la posición en el eje x a la figura dibujada, está constantemente afectada por la variable velocidad en nuestro bloque `draw()`. De esta forma, la posición del círculo se devuelve en cada frame y la posición anterior es borrada al redibujar el fondo con la función `background()`, dando así la sensación de movimiento.
...
function draw() {
  background(0);
  let y1 = desplazamiento + sin(angulo) * escalar;
  let y2 = desplazamiento + sin(angulo + 0.4) * escalar;
  let y3 = desplazamiento + sin(angulo + 0.8) * escalar;
  ellipse( 80, y1, 40, 40);
