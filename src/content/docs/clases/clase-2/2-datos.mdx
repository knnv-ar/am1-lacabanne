...
**Recomendación oficial de p5.js:**
> Evitamos usar `var` para declarar variables. Siguiendo la [guía de estilo oficial de p5.js](https://p5js.org/contribute/documentation_style_guide/), las variables declaradas con `var` tienen reglas de alcance (scoping rules) confusas que pueden derivar en errores sutiles (bugs). Por ello, en toda esta documentación utilizamos `let`.
...
```javascript
let x = 51; //La variable d es global, puede ser usada donde sea
function setup() {
  createCanvas(x, 100);
