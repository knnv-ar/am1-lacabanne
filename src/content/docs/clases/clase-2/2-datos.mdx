...
Cuando declaras una variable, usas la palabra let, para indicar que estás creando una nueva variable, seguida del nombre. Después de que el nombre es fijado, se le puede asignar un valor a la variable:  
...
```javascript
let x = 12; // Declara la variable x y le asigna un valor 
```

La palabra let en la línea de código que declara la variable se escribe una sola vez, no se debe escribir de nuevo al utilizarla o al asignarle otro valor. Cada vez que se escribe let antes del nombre de una variable, la computadora piensa que estás tratando de declarar una nueva variable. Por lo tanto, no podés tener dos variables con el mismo nombre en la misma sección del programa, ya que el programa devolverá un error: 

```javascript
let x; // Declara la variable x 
let x = 12; // ¡ERROR! Ya existe una variable x 
```

> **Importante:** Anteriormente en JavaScript era común declarar variables usando la palabra clave `var`. Sin embargo, de acuerdo con la [guía de estilo de documentación de p5.js](https://p5js.org/contribute/documentation_style_guide/), se recomienda evitar el uso de `var` y se recomienda usar `let`:
...
```javascript
let x = 51; //La variable x es global, puede ser usada donde sea
function setup() {
  createCanvas(x, 100);
  let valor = 20; //La variable local valor, puede ser usada solo en el setup()
  fill(valor); 
}
function draw() {
  let y = 60; //La variable local y, puede ser usada solo en el draw()
  line(0, y, x, y);
  y = 25;
  line(0, y, x, y);
}
...
Cuando nos referimos a estructuras debemos tener en cuenta que no existen solo las que ya conocemos `setup()` y `draw()` sino todas aquellas que tengan llave de apertura y cierre como pueden ser `if(){}` y `for{}` que veremos más adelante. Al declarar variables con `let`, éstas tienen un ámbito de bloque (block scope). Esto significa que si declaramos una variable con `let` dentro de una estructura, esa variable solo existe dentro de ese bloque y no puede ser utilizada por fuera de él. Anteriormente se utilizaba `var` para declarar variables, pero `var` tiene un ámbito de función (function scope) lo que puede provocar comportamientos confusos e inesperados. Por esta razón, la [guía de estilo de documentación de p5.js](https://p5js.org/contribute/documentation_style_guide/) indica evitar el uso de `var` y usar `let` para declarar variables, ya que `let` proporciona un ámbito de bloque más predecible y seguro.
